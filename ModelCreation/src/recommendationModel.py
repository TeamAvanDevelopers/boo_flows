...
class TaggedDocuments(object):
    ...
    def get_custom_tagged_document(self, defined_data_set):
        from gensim.models.doc2vec import TaggedDocument
        custom_tagged_document = []
        for idx in defined_data_set.keys():
            split_tags = defined_data_set[idx].split(" ")
...
def save_vocab(vocab_name, save_root, vocab_type):
    ...
    if lower_vocab_type in vocab_types:
        ...
        print("Check your vocab type. You can use only 4 types like user2vec, post2vec, tag2Vec, relatedtag2vec")
...
def save_model(model, save_root, model_type):
    ...
    if lower_model_type in model_types:
        ...
        print("Check your model type. You can use only 4 types like user2vec, post2vec, tag2Vec, relatedtag2vec")
...
def load_model(load_path):
    if load_path.find('user2vec') > 0 or load_path.find('post2vec') > 0:
        ...
        print("Check your path! The current path is {}".format(load_path))
...
def infer_item(model, labeled_document):
    pass
